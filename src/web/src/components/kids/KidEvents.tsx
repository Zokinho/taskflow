...
import { useKidEvents } from '@/hooks/useKids';
...
export function KidEvents({ kidId }: { kidId: string }) {
  const [from, setFrom] = useState('');
  const [to, setTo] = useState('');
  const { data: events, isLoading } = useKidEvents(kidId, from || undefined, to || undefined);
